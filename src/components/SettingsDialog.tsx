...
const menuItems = [
    ...
    { id: 'regions', label: '地区管理', icon: MapPin },
];
...
export function SettingsDialog({ open, onOpenChange, onRegionsChange }: SettingsDialogProps) {
    ...
    // Render Regions Panel
    const renderRegionsPanel = () => (
        <div className="h-full flex flex-col">
            {/* Selected Tags */}
            {selectedRegions.length > 0 && (
                <div className="flex flex-wrap gap-1.5 p-2 border-b mb-2 shrink-0">
            ...
            <div className="flex-1 grid grid-cols-2 gap-4 overflow-hidden min-h-0">
                        ...
                        {searchResults.length > 0 ? (
                            searchResults.map(r => (
                                <div
                                    ...
                                    className={cn(
                                        "flex items-center justify-between py-1.5 px-2 rounded cursor-pointer text-sm",
                ...
                {/* Browse */}
                <div className="flex flex-col overflow-hidden">
                    <div className="text-sm font-medium mb-2 shrink-0">浏览地区</div>
                    <div className="flex-1 overflow-y-auto border rounded p-1">
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col p-0">
                ...
                <div className="flex flex-1 overflow-hidden">
                    {/* Left Menu */}
                    <nav className="w-40 border-r bg-muted/30 p-2 shrink-0">
                        {menuItems.map((item) => {
                    ...
                    <div className="flex-1 p-4 overflow-y-auto">
                        {activeMenu === 'apikeys' && renderApiKeysPanel()}
                        {activeMenu === 'regions' && renderRegionsPanel()}
                    </div>
                </div>
            </DialogContent>
        </Dialog>
