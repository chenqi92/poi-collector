...
多源地理数据采集工具，支持 POI（兴趣点）批量采集、航标数据采集、AIS 航迹分析、水域面过滤、多平台瓦片地图下载，以及数据管理与导出。
...
![AIS 航迹分析](docs/img/ais.png)

| 水域面效果                                                     | AIS 水域面过滤                                                    |
|------------------------------------------------------------|---------------------------------------------------------------|
| <img src="docs/img/水域面效果.png" alt="水域面效果" width="100%"> | <img src="docs/img/水域面过滤.png" alt="AIS 水域面过滤" width="100%"> |
...
### 🚢 AIS 航迹与水域面过滤
- AIS 航迹可视化展示，支持航向箭头、轨迹点聚合与停泊段识别
- 支持叠加长江航道图 `HYDRO_A` 水域面，辅助复核船舶位置
- 基于水域面空间关系识别水域内、近岸、水域外和未知覆盖状态
- 可标记水域外异常 AIS 点，保留过滤原因便于后续复核
