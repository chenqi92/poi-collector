...
impl TileStorage for FolderStorage {
    fn init(
        ...
        // 创建基础目录
        fs::create_dir_all(&self.base_path).map_err(|e| format!("创建目录失败: {}", e))?;
    ...
    fn save_tile(&mut self, coord: &TileCoord, data: &[u8]) -> Result<(), String> {
        ...
        fs::create_dir_all(&tile_dir).map_err(|e| format!("创建瓦片目录失败: {}", e))?;

        // 保存瓦片文件 y.png
        let tile_path = tile_dir.join(format!("{}.png", coord.y));
        fs::write(&tile_path, data).map_err(|e| format!("保存瓦片失败: {}", e))?;
